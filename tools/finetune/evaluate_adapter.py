...
Runs inside unsloth-gemma4-local:latest Docker container with bind mounts:
  ...
  /workspace/output/          ← all outputs written here
"""
...
OUTPUTS_JSONL = OUTPUT_DIR / "eval_outputs.jsonl"
REPORT_MD    = OUTPUT_DIR / "eval_report.md"
METRICS_JSON = OUTPUT_DIR / "eval_metrics.json"
...
def _analyse_behavior(outputs: list[dict]) -> str:
    """Return a brief paragraph of observed adapter behavior patterns."""
    notes = []
...
def main() -> None:
    ...
    # --- Write metrics.json ---
    metrics = {
        "model": f"{BASE_MODEL_ID} + LoRA(r=16, a=32)",
